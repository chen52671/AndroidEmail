...
public class Conversation implements Parcelable {
     ...
     */
    public static final String UPDATE_FOLDER_COLUMN = UIProvider.ConversationColumns.RAW_FOLDERS;
    ...
    public void applyCachedValues(ContentValues values) {
        ...
        for (String key : values.keySet()) {
            ...
            LogUtils.i(LOG_TAG, "Conversation: applying cached value to col=%s val=%s", key,
                    val);
            if (UIProvider.ConversationColumns.READ.equals(key)) {
                read = (Integer) val != 0;
            } else if (UIProvider.ConversationColumns.CONVERSATION_INFO.equals(key)) {
                conversationInfo = ConversationInfo.fromBlob((byte[]) val);
            } else if (UIProvider.ConversationColumns.FLAGS.equals(key)) {
                convFlags = (Integer) val;
            } else if (UIProvider.ConversationColumns.STARRED.equals(key)) {
                starred = (Integer) val != 0;
            } else if (UIProvider.ConversationColumns.SEEN.equals(key)) {
                seen = (Integer) val != 0;
            } else if (UIProvider.ConversationColumns.RAW_FOLDERS.equals(key)) {
                rawFolders = FolderList.fromBlob((byte[]) val);
            } else if (UIProvider.ConversationColumns.VIEWED.equals(key)) {
