...
public class Settings implements Parcelable {
     ...
     * Integer, one of {@link UIProvider.AutoAdvance#LIST}, {@link UIProvider.AutoAdvance#NEWER},
     * {@link UIProvider.AutoAdvance#OLDER} or  {@link UIProvider.AutoAdvance#UNSET}
     */
    ...
    private Settings() {
        signature = "";
        mAutoAdvance = UIProvider.AutoAdvance.LIST;
        messageTextSize = UIProvider.MessageTextSize.NORMAL;
        snapHeaders = UIProvider.SnapHeaderValue.ALWAYS;
        replyBehavior = UIProvider.DefaultReplyBehavior.REPLY;
        convListIcon = UIProvider.ConversationListIcon.SENDER_IMAGE;
    ...
    public Settings(Cursor cursor) {
        signature = cursor.getString(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.SIGNATURE));
        mAutoAdvance = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.AUTO_ADVANCE));
        messageTextSize = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.MESSAGE_TEXT_SIZE));
        snapHeaders = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.SNAP_HEADERS));
        replyBehavior = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.REPLY_BEHAVIOR));
        convListIcon = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.CONV_LIST_ICON));
        convListAttachmentPreviews = cursor
                .getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.CONV_LIST_ATTACHMENT_PREVIEWS)) != 0;
        confirmDelete = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.CONFIRM_DELETE)) != 0;
        confirmArchive = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.CONFIRM_ARCHIVE)) != 0;
        confirmSend = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.CONFIRM_SEND)) != 0;
        defaultInbox = Utils.getValidUri(
                cursor.getString(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.DEFAULT_INBOX)));
        defaultInboxName =
                cursor.getString(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.DEFAULT_INBOX_NAME));
        forceReplyFromDefault = cursor.getInt(
                cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.FORCE_REPLY_FROM_DEFAULT)) != 0;
        maxAttachmentSize =
                cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.MAX_ATTACHMENT_SIZE));
        swipe = cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.SWIPE));
        priorityArrowsEnabled = cursor.getInt(
                cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.PRIORITY_ARROWS_ENABLED)) != 0;
        setupIntentUri = Utils.getValidUri(
                cursor.getString(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.SETUP_INTENT_URI)));
        conversationViewMode =
                cursor.getInt(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.CONVERSATION_VIEW_MODE));
        veiledAddressPattern =
                cursor.getString(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.VEILED_ADDRESS_PATTERN));
        moveToInbox = Utils.getValidUri(
                cursor.getString(cursor.getColumnIndex(UIProvider.AccountColumns.SettingsColumns.MOVE_TO_INBOX)));
    }

    private Settings(JSONObject json) {
        signature = json.optString(UIProvider.AccountColumns.SettingsColumns.SIGNATURE, sDefault.signature);
        mAutoAdvance = json.optInt(UIProvider.AccountColumns.SettingsColumns.AUTO_ADVANCE, sDefault.getAutoAdvanceSetting());
        messageTextSize = json.optInt(UIProvider.AccountColumns.SettingsColumns.MESSAGE_TEXT_SIZE, sDefault.messageTextSize);
        snapHeaders = json.optInt(UIProvider.AccountColumns.SettingsColumns.SNAP_HEADERS, sDefault.snapHeaders);
        replyBehavior = json.optInt(UIProvider.AccountColumns.SettingsColumns.REPLY_BEHAVIOR, sDefault.replyBehavior);
        convListIcon = json.optInt(UIProvider.AccountColumns.SettingsColumns.CONV_LIST_ICON, sDefault.convListIcon);
        convListAttachmentPreviews = json.optBoolean(UIProvider.AccountColumns.SettingsColumns.CONV_LIST_ATTACHMENT_PREVIEWS,
                sDefault.convListAttachmentPreviews);
        confirmDelete = json.optBoolean(UIProvider.AccountColumns.SettingsColumns.CONFIRM_DELETE, sDefault.confirmDelete);
        confirmArchive = json.optBoolean(UIProvider.AccountColumns.SettingsColumns.CONFIRM_ARCHIVE, sDefault.confirmArchive);
        confirmSend = json.optBoolean(UIProvider.AccountColumns.SettingsColumns.CONFIRM_SEND, sDefault.confirmSend);
        defaultInbox = Utils.getValidUri( json.optString(UIProvider.AccountColumns.SettingsColumns.DEFAULT_INBOX));
        defaultInboxName = json.optString(UIProvider.AccountColumns.SettingsColumns.DEFAULT_INBOX_NAME,
                sDefault.defaultInboxName);
        forceReplyFromDefault = json.optBoolean(UIProvider.AccountColumns.SettingsColumns.FORCE_REPLY_FROM_DEFAULT,
                sDefault.forceReplyFromDefault);
        maxAttachmentSize =
                json.optInt(UIProvider.AccountColumns.SettingsColumns.MAX_ATTACHMENT_SIZE, sDefault.maxAttachmentSize);
        swipe = json.optInt(UIProvider.AccountColumns.SettingsColumns.SWIPE, sDefault.swipe);
        priorityArrowsEnabled = json.optBoolean(UIProvider.AccountColumns.SettingsColumns.PRIORITY_ARROWS_ENABLED,
                sDefault.priorityArrowsEnabled);
        setupIntentUri = Utils.getValidUri(json.optString(UIProvider.AccountColumns.SettingsColumns.SETUP_INTENT_URI));
        conversationViewMode = json.optInt(UIProvider.AccountColumns.SettingsColumns.CONVERSATION_VIEW_MODE,
                UIProvider.ConversationViewMode.UNDEFINED);
        veiledAddressPattern = json.optString(UIProvider.AccountColumns.SettingsColumns.VEILED_ADDRESS_PATTERN, null);
        moveToInbox = Utils.getValidUri(json.optString(UIProvider.AccountColumns.SettingsColumns.MOVE_TO_INBOX));
    }
    ...
    public synchronized JSONObject toJSON() {
        ...
        try {
            json.put(UIProvider.AccountColumns.SettingsColumns.SIGNATURE, getNonNull(signature, sDefault.signature));
            json.put(UIProvider.AccountColumns.SettingsColumns.AUTO_ADVANCE, getAutoAdvanceSetting());
            ...
            json.put(UIProvider.AccountColumns.SettingsColumns.CONV_LIST_ICON, convListIcon);
            json.put(UIProvider.AccountColumns.SettingsColumns.CONV_LIST_ATTACHMENT_PREVIEWS, convListAttachmentPreviews);
            ...
            json.put(UIProvider.AccountColumns.SettingsColumns.DEFAULT_INBOX,
                    getNonNull(defaultInbox, sDefault.defaultInbox));
            json.put(UIProvider.AccountColumns.SettingsColumns.DEFAULT_INBOX_NAME,
                    getNonNull(defaultInboxName, sDefault.defaultInboxName));
            json.put(UIProvider.AccountColumns.SettingsColumns.FORCE_REPLY_FROM_DEFAULT, forceReplyFromDefault);
            json.put(UIProvider.AccountColumns.SettingsColumns.MAX_ATTACHMENT_SIZE,  maxAttachmentSize);
            json.put(UIProvider.AccountColumns.SettingsColumns.SWIPE, swipe);
            json.put(UIProvider.AccountColumns.SettingsColumns.PRIORITY_ARROWS_ENABLED, priorityArrowsEnabled);
            json.put(UIProvider.AccountColumns.SettingsColumns.SETUP_INTENT_URI, setupIntentUri);
            json.put(UIProvider.AccountColumns.SettingsColumns.CONVERSATION_VIEW_MODE, conversationViewMode);
            json.put(UIProvider.AccountColumns.SettingsColumns.VEILED_ADDRESS_PATTERN, veiledAddressPattern);
            json.put(UIProvider.AccountColumns.SettingsColumns.MOVE_TO_INBOX,
     ...
     * a null object. It always returns a valid {@link UIProvider.Swipe} setting.
     * @return the auto advance setting, a constant from {@link UIProvider.Swipe}
     */
