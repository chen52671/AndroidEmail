...
public class Folder implements Parcelable, Comparable<Folder> {
    ...
    public static void setFolderBlockColor(Folder folder, View colorBlock) {
        ...
        boolean showBg =
                !TextUtils.isEmpty(folder.bgColor) && (folder.type & UIProvider.FolderType.INBOX_SECTION) == 0;
    ...
    /**
     * Checks if <code>typeMask</code> is of the specified {@link UIProvider.FolderType}
     *
     * @return <code>true</code> if the mask contains the specified
     *         {@link UIProvider.FolderType}, <code>false</code> otherwise
     */
    ...
    public String getTypeDescription() {
        final String desc;
        if (isType(UIProvider.FolderType.INBOX_SECTION)) {
            ...
            desc = "important";
        } else if (isType(UIProvider.FolderType.OUTBOX)) {
            desc = "outbox";
        } else if (isType(UIProvider.FolderType.SENT)) {
            desc = "sent";
        } else if (isType(UIProvider.FolderType.SPAM)) {
            desc = "spam";
        } else if (isType(UIProvider.FolderType.STARRED)) {
            desc = "starred";
        } else if (isTrash()) {
            desc = "trash";
        } else if (isType(UIProvider.FolderType.UNREAD)) {
            desc = "unread";
        } else if (isViewAll()) {
            desc = "all_mail";
    ...
    public final boolean isUnreadCountHidden() {
        return (isDraft() || isTrash() || isType(UIProvider.FolderType.OUTBOX));
    }
