...
package com.chen.emailcommon.provider;
...
import com.chen.emailcommon.utility.TextUtilities;
...
public final class Policy extends EmailContent implements EmailContent.PolicyColumns, Parcelable {
    ...
    public static final String[] CONTENT_PROJECTION = new String[] {RECORD_ID,
            PASSWORD_MODE, PASSWORD_MIN_LENGTH,
            PASSWORD_EXPIRATION_DAYS, PASSWORD_HISTORY,
            PASSWORD_COMPLEX_CHARS, PASSWORD_MAX_FAILS,
            MAX_SCREEN_LOCK_TIME, REQUIRE_REMOTE_WIPE,
            REQUIRE_ENCRYPTION, REQUIRE_ENCRYPTION_EXTERNAL,
            REQUIRE_MANUAL_SYNC_WHEN_ROAMING, DONT_ALLOW_CAMERA,
            DONT_ALLOW_ATTACHMENTS, DONT_ALLOW_HTML,
            MAX_ATTACHMENT_SIZE, MAX_TEXT_TRUNCATION_SIZE,
            MAX_HTML_TRUNCATION_SIZE, MAX_EMAIL_LOOKBACK,
            MAX_CALENDAR_LOOKBACK, PASSWORD_RECOVERY_ENABLED,
            PROTOCOL_POLICIES_ENFORCED, PROTOCOL_POLICIES_UNSUPPORTED
    };

    public static final Policy NO_POLICY = new Policy();

    private static final String[] ATTACHMENT_RESET_PROJECTION =
        new String[] {RECORD_ID, AttachmentColumns.SIZE, AttachmentColumns.FLAGS};
    ...
    public static Policy restorePolicyWithId(Context context, long id) {
        return restoreContentWithId(context, Policy.class, Policy.CONTENT_URI,
                Policy.CONTENT_PROJECTION, id);
    }

    public static long getAccountIdWithPolicyKey(Context context, long id) {
        return Utility.getFirstRowLong(context, Account.CONTENT_URI, ID_PROJECTION,
                AccountColumns.POLICY_KEY + "=?", new String[] {Long.toString(id)}, null,
                ID_PROJECTION_COLUMN, Account.NO_ACCOUNT);
    }
    ...
    public ContentValues toContentValues() {
        ...
        values.put(PASSWORD_MODE, mPasswordMode);
        values.put(PASSWORD_MIN_LENGTH, mPasswordMinLength);
        values.put(PASSWORD_MAX_FAILS, mPasswordMaxFails);
        values.put(PASSWORD_HISTORY, mPasswordHistory);
        values.put(PASSWORD_EXPIRATION_DAYS, mPasswordExpirationDays);
        values.put(PASSWORD_COMPLEX_CHARS, mPasswordComplexChars);
        values.put(MAX_SCREEN_LOCK_TIME, mMaxScreenLockTime);
        values.put(REQUIRE_REMOTE_WIPE, mRequireRemoteWipe);
        values.put(REQUIRE_ENCRYPTION, mRequireEncryption);
        values.put(REQUIRE_ENCRYPTION_EXTERNAL, mRequireEncryptionExternal);
        values.put(REQUIRE_MANUAL_SYNC_WHEN_ROAMING, mRequireManualSyncWhenRoaming);
        values.put(DONT_ALLOW_CAMERA, mDontAllowCamera);
        values.put(DONT_ALLOW_ATTACHMENTS, mDontAllowAttachments);
        values.put(DONT_ALLOW_HTML, mDontAllowHtml);
        values.put(MAX_ATTACHMENT_SIZE, mMaxAttachmentSize);
        values.put(MAX_TEXT_TRUNCATION_SIZE, mMaxTextTruncationSize);
        values.put(MAX_HTML_TRUNCATION_SIZE, mMaxHtmlTruncationSize);
        values.put(MAX_EMAIL_LOOKBACK, mMaxEmailLookback);
        values.put(MAX_CALENDAR_LOOKBACK, mMaxCalendarLookback);
        values.put(PASSWORD_RECOVERY_ENABLED, mPasswordRecoveryEnabled);
        values.put(PROTOCOL_POLICIES_ENFORCED, mProtocolPoliciesEnforced);
        values.put(PROTOCOL_POLICIES_UNSUPPORTED, mProtocolPoliciesUnsupported);
        return values;
    }
