...
import com.chen.mail.widget.WidgetService;

public class WidgetProvider extends BaseWidgetProvider {
    private static final String LEGACY_PREFS_NAME = "com.chen.email.widget.WidgetManager";
    ...
    @Override
    protected com.chen.mail.providers.Account getAccountObject(
    ...
    @Override
    protected boolean isAccountValid(Context context, com.chen.mail.providers.Account account) {
        if (account != null) {
            final ContentResolver resolver = context.getContentResolver();
    ...
    protected void migrateLegacyWidgetInformation(Context context, int widgetId) {
        ...
        final com.chen.mail.providers.Account uiAccount = getAccount(context, accountId);
    ...
    private static com.chen.mail.providers.Account getAccount(Context context, long accountId) {
        final ContentResolver resolver = context.getContentResolver();
        final Cursor ac = resolver.query(EmailProvider.uiUri("uiaccount", accountId),
                UIProvider.ACCOUNTS_PROJECTION_NO_CAPABILITIES, null, null, null);

        com.chen.mail.providers.Account uiAccount = getPopulatedAccountObject(ac);

        return uiAccount;
    }

    private static com.chen.mail.providers.Account getPopulatedAccountObject(
